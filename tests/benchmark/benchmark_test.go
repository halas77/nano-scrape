...
func loadBenchmarkTag(b *testing.B) *nano.Tag {
	b.Helper()
...
func BenchmarkFindPath(b *testing.B) {
	...
	name := "span"
	attrs := []*nano.Attribute{{Key: "id", Value: "category-item"}}
	b.ResetTimer()
	for b.Loop() {
		tag.Find(name, attrs, func(t *nano.Tag) {})
	}
}
...
func BenchmarkFindAllPath(b *testing.B) {
	tag := loadBenchmarkTag(b)
	name := "div"
	attrs := []*nano.Attribute{{Key: "class", Value: "category-item"}}
	b.ResetTimer()
	for b.Loop() {
